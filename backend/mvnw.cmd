...
# Auto-detect or validate JAVA_HOME for Windows environments
if ($env:JAVA_HOME -and (Test-Path "$env:JAVA_HOME\bin\java.exe")) {
    Write-Output "JAVA_HOME=$env:JAVA_HOME"
} else {
    $searchPaths = @(
        "C:\Users\$env:USERNAME\Downloads\jdk-*\jdk-*\bin\java.exe",
        "C:\Users\$env:USERNAME\Downloads\jdk-*\bin\java.exe",
    ...
    foreach ($pattern in $searchPaths) {
        ...
        if ($match) {
            $foundJdk = (Get-Item $match.Path).Directory.Parent.FullName
            break
        }
    }
    if ($foundJdk) {
        $env:JAVA_HOME = $foundJdk
        Write-Output "JAVA_HOME=$foundJdk"
    }
}
